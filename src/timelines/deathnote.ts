...
const CHAPTER_LINK_EXCEPTIONS = new Set([
    'Boredom',
    'Matsuda',
    'Give-and-Take',
    'Confluence',
...
const EPISODE_LINK_EXCEPTIONS = new Set([
    'Love',
    'Decision',
    'New World',
    'Matsuda',
    ...
    layout: {
        season: {
            ...
            subTimeline: {
                ...
                sectionLink: 'Death Note (anime)',
                wikiLink: title =>
                    EPISODE_LINK_EXCEPTIONS.has(title) ?
                        `${title} (episode)`
                    :   title,
                focusable: true,
            },
        ...
        chapter: {
            ...
            wikiLink: (title: string, n: number): string =>
                ...
                : CHAPTER_LINK_EXCEPTIONS.has(title) ? `${title} (chapter)`
                : n === 62 ? 'The Decision'
                : title,
            focusable: true,
        },
        volume: {
            ...
            sectionLink: 'List of Death Note chapters#List_of_volumes',
            wikiLink: title =>
                CHAPTER_LINK_EXCEPTIONS.has(title) ?
                    `${title} (volume)`
                :   title,
        },
    },
